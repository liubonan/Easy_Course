...
List<Teachingstaff> ts_list = (List<Teachingstaff>)session.getAttribute("tss");
WebHelper wh =new WebHelper();%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Browse Staff</title>
    ...
    <div class="blank30"></div>
        <div class="grid_620 min_600">
            <div class="img_grid_left">
                ...
                <%int size_y=(int)ts_list.size()/6; %>
                <%int remainder=ts_list.size()%6; %>
                <%for (int j=0;j<size_y;j++){ %>
                <ol class="img_grid">
                	<%for(int i = 0;i < 6;i++){ %>
                	<li>
                		<div class="img_grid_container">
                			<a href="/EasyCourse/StaffServlet?tsid=<%=ts_list.get(j*6+i).getTsid()%>"><img src="images/staff.jpg"/></a>
                		</div>
                    	<div class="blank5"></div>
                        <p><a href="/EasyCourse/StaffServlet?tsid=<%=ts_list.get(j*6+i).getTsid()%>"><%=ts_list.get(j*6+i).getTsname() %></a></p>
                    </li>   
                    <%} %>                 
                </ol>
                <div class="blank10"></div>
                ...
                <ol class="img_grid">
                	...
                	<li>
                		<div class="img_grid_container">
                			<a href="/EasyCourse/StaffServlet?tsid=<%=ts_list.get(size_y*6+i).getTsid()%>"><img src="images/staff.jpg"/></a>
                		</div>
                    	<div class="blank5"></div>
                        <p><a href="/EasyCourse/StaffServlet?tsid=<%=ts_list.get(size_y*6+i).getTsid()%>"><%=ts_list.get(size_y*6+i).getTsname() %></a></p>
                    </li>   
                	<%} %>
                </ol>
                <%} %>        
                <%}else{ %>
                <h3>There is no staffs now.</h3>
                <%} %>
            </div>
            <div class="blank30"></div>
        </div>
        <div class="grid_300">
        	...
        	<p class="link_blue">+&nbsp;<a href="/EasyCourse/addstaff.jsp">add new teaching staff</a></p>
        </div>
        <div class="blank30"></div>
    </div>
